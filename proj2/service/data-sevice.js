...
import { Dron } from '../src/dron.js';
import { DataErrors } from './data-error.js';
...
export class DataService {
            ...
            switch (data.type) {
                ...
                case 'car':
                    if (this.validateCarData(data)) {
                        ...
                        let e = new DataErrors('invalid data at car case', data);
                        this.errors.push(e);
                    }
                    break;
                ...
                default:
                    let e = new DataErrors('invalid data at default case', data);
                    this.errors.push(e);
                    break;
    ...
    loadCar(car) {
        try {
            ...
            this.errors.push(new DataErrors('car error', car));
        }
        return null;
    }
    ...
    loadDron(dron) {
        try {
            let d = new Dron(dron.license, dron.model, dron.latLong);
            d.airTimeHours = dron.airTimeHours;
            d.base = dron.base;
            return d;
        } catch (e) {
            this.errors.push(new DataErrors('dron error', dron));
        }
        return null;
    }
    ...
    validateCarData(car) {
        ...
        for (let field of requiredetail) {
            if (!car[field]) {
                this.errors.push(new DataErrors(`invalid field in validatin ${field}`, car));
            }
        }
        return car;
    }
    ...
    validateDroneData(dron) {
        ...
        for (let field of requiredetail) {
            if (!dron[field]) {
                this.errors.push(new DataErrors(`invalid field ${field}`, dron));
            }
        }
        return dron;
    }
